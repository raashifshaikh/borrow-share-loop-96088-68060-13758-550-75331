...
const Messages = () => {
  ...
  // Fetch messages and negotiations
                    ...
                    {messages.map((msg, index) => {
                      ...
                      if (msg.type === 'negotiation') {
                        const displayText = msg.message
                          ? msg.message
                          : msg.amount
                            ? `Offer: $${parseFloat(msg.amount).toFixed(2)}`
                            : 'Offer sent';

                        return (
                          <NegotiationMessage
                            key={msg.id}
                            negotiation={{ ...msg, displayText }}
...
export default Messages;  const { data: conversations, isLoading: conversationsLoading } = useQuery({
    queryKey: ['conversations', user?.id],
    queryFn: async () => {
      if (!user?.id) return [];
